...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  let userRecord;
  try {
    // First, try to authenticate with Firebase Auth REST API
    const axios = require('axios');

    // Use Firebase Auth REST API to verify email/password
    // Note: You need to set FIREBASE_WEB_API_KEY in your environment
    ...
    if (firebaseWebApiKey) {
      ...
      logger.info(`Firebase Auth verification successful for ${email}`);
    } else {
      logger.warn('FIREBASE_WEB_API_KEY not set, skipping password verification');
    }
    ...
    if (error.response?.data?.error?.message) {
      logger.warn(`Login failed for ${email}: ${error.response.data.error.message}`);
    } else {
  ...
  if (snapshot.empty) {
    logger.warn(`Login failed: No user found in Firestore for ${email}`);
    throw new AppError('Invalid credentials', 401);
  }
  ...
  // Set cookie options for JWT if needed
  const cookieOptions = {
    expires: new Date(
  ...
  req.session.save(err => {
    ...
    // Optionally set JWT cookie as well
